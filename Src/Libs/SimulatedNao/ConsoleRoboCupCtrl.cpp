...
bool ConsoleRoboCupCtrl::compile()
{
  ...
  executeFile("", fileName, false, nullptr);
...
void ConsoleRoboCupCtrl::executeConsoleCommand(std::string command, RobotConsole* console)
{
  ...
  else if(buffer == "gc")
  {
    ...
    if(buffer == "initial")
      ...
      check(gameController.globalGameStuck(1));
      ...
      check(gameController.penaltyKick(1));
...
bool ConsoleRoboCupCtrl::startRemote(In& stream)
{
  ...
  RemoteConsole* rc = new RemoteConsole(name, ip, this);
  if(!rc->isClient())
  {
    if(ip != "")
    {
...
void ConsoleRoboCupCtrl::createCompletion()
{
  const char* commands[] =
  {
    ...
    "gc globalGameStuckBySecondTeam",
    "gc goalByFirstTeam",
    "gc goalBySecondTeam",
